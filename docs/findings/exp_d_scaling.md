...
- k=5: n > ~10 (fifth root of 100k), so k=5 is impossible for any interesting n

## Implications for the Sutro Group

1. **GrokFast may help scaling**: The 50-100x speedup from GrokFast (exp4) would push the k=3 frontier from n~46 to n~200+. For k=5, even GrokFast may not suffice.
...
3. **Energy-efficient algorithms become necessary at scale**: At k=5, the n^k barrier means standard SGD wastes energy on steps that make no visible progress (the "hidden progress" regime). Reducing per-step cost (better ARD) compounds with reducing step count.
